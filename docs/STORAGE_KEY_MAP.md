...
`authedApiRequest()` clears these credentials on a server `401`. `destroyAllData()` now also calls `clearCredentials()` because nuclear reset is treated as a fresh local app state boundary. Ordinary vault lock does not clear these credentials.
