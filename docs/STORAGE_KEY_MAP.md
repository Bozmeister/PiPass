...
| Local API credentials | SecureStore/localStorage | `pipass.auth.userId`, `pipass.auth.authHash` | Authentication secret material | Server auth credential rotation must replace `authHash`; logout/reset ownership needs confirmation |
...
| `deviceUUID` | `crypto/keyDerivation.ts` | Per-device UUID mixed into local KDF material | Sensitive local derivation input | No for existing vaults; changing it can make old vault data undecryptable | Should not change during ordinary password rotation unless a migration handles it | No | Needs explicit migration decision | No explicit wipe in `destroyAllData()` found |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `pipass.auth.userId` | Server user UUID | Sensitive account identifier | Yes after login/register | No | Usually unchanged | No | Deleted by `clearCredentials()` |
| `pipass.auth.authHash` | Client auth credential proof | Authentication secret material | Only from login/credential derivation | Maybe if combined password/auth flow rotates it | Must replace after server confirms new credential | No | Deleted by `clearCredentials()` |

`authedApiRequest()` clears these credentials on a server `401`. The current vault `destroyAllData()` path does not call `clearCredentials()`. That ownership should be decided before account reset or password/auth rotation work.
...
| Key | Owner | Stored data | Sensitivity | Regenerable | Password rotation | Auth rotation | Root-key migration | Wipe behavior |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `pipass.installId` | `lib/install-id.ts` | Random UUID label | Non-secret, spoofable audit label | Yes | No | No | No | No explicit wipe found |

`installId` must remain label-only. It is not an auth factor, trust proof, recovery key, KDF input, or vault-access decision.
...
| `destroyAllData()` | Calls `clearVault()` and `clearAllNotes()`, then deletes master salt, security profile, show keyprints, initialized flag, fractal fingerprint, recovery hash, migration flag, and cached master key | Does not delete `pipass.auth.*`, `pipass.installId`, or `deviceUUID` |
| `clearCredentials()` | Deletes `pipass.auth.userId` and `pipass.auth.authHash` | Not called by `destroyAllData()` in current storage worker |
...
Before implementing account reset or password/auth rotation, decide whether local API credentials, `installId`, and `deviceUUID` should be wiped by that flow.
...
- whether `pipass.auth.*` is updated or cleared
...
- `deviceUUID` is a local KDF input but is owned by `crypto/keyDerivation.ts`, not `workers/storageWorker.ts`. It has no clear wipe/export/migration API.
- `pipass.auth.*` credentials are separate from vault reset and are not cleared by `destroyAllData()`.
- `pipass.installId` is separate from reset and may intentionally persist, but that should be a product decision.
