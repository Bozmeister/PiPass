...
| --- | --- | --- | --- | --- |
| Master unlock metadata | SecureStore on native, localStorage on web | `pipass_master_salt`, `pipass_master_hash`, `pipass_security_profile`, `pipass_kdf_metadata`, `deviceUUID` | Sensitive metadata | Master password rotation must update salt/hash/profile/KDF metadata carefully; `deviceUUID` must remain stable unless migrating intentionally |
...
| `pipass_kdf_metadata` | `workers/storageWorker.ts` | JSON metadata describing the actual local KDF algorithm and parameters for future unlock migration/rotation work | Sensitive metadata, not secret by itself | Yes only after successful unlock verification or staged rotation | Must update only through staged password/profile rotation | No | May become password-wrapper KDF metadata | Preserved by `clearVault()` for now; deleted by `destroyAllData()` |
