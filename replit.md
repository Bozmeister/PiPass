...
- **Android AutoFill Service** (`native/android/java/com/pipass/app/autofill/` → copied to `android/app/src/main/java/com/pipass/app/autofill/` by config plugin during prebuild): `PiPassAutofillService.kt` implements `AutofillService`, reads vault and master key from EncryptedSharedPreferences (file: `group.com.pipass.shared`, keys: `pipass_shared_vault` / `pipass_master_key`) using default MasterKey (AES256_GCM, default alias), per-entry HKDF key derivation via `CryptoHelper.deriveEntryKey()`, HMAC verify then AES-CBC decrypt via `verifyAndDecryptBytes()`, domain matching from AssistStructure, builds Dataset objects with AutofillValue for username/password fields. `CryptoHelper.kt` provides JCE-based crypto (AES/CBC/PKCS5Padding, HmacSHA256, SHA-256). All byte arrays wiped via `wipeBytes()` in `finally` blocks. 30s session timeout with Handler-based auto-wipe. Credential ranking: +100 exact domain, +50 subdomain, +20 recent, +10 short distance; max 3 results; fallback to most recent entries. Field detection: 3-tier — (1) autofillHints, (2) InputType variation masking (`0x00000FF0`), (3) viewId/hint text fallback (`node.idEntry` and `node.hint` checked for username/email/password/login keywords). All error paths call `callback.onSuccess(null)` (never `onFailure`) to prevent system-level error popups. `onSaveRequest` extracts and logs entered username/password from AssistStructure using the same 3-tier field detection. SaveInfo attached to FillResponse with `SAVE_DATA_TYPE_USERNAME | SAVE_DATA_TYPE_PASSWORD`. Custom RemoteViews layout (`res/layout/autofill_item.xml`) with title and subtitle; falls back to `simple_list_item_1` if layout unavailable. Lifecycle: `onConnected()`, `onDisconnected()` (wipes session), `onDestroy()` (wipes session).
