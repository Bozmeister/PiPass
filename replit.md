...
- **API Routes** (`server/routes.ts`):
  ...
  - `POST /api/vault/sync` — Upload encrypted vault blob with `{encryptedBlob, expectedPrevVersion}`. The server mints the next stored version from current server state; request bodies containing `version` are rejected. A stale `expectedPrevVersion` returns 409 `{error: "Version conflict", serverVersion}`. Returns 423 (recovery mode), 423 (soft-lock), or 403 (untrusted device) before write.
...
### Client API Helper (`lib/`)
- **`lib/credentials.ts`** — single source of truth for the auth bearer credential pair `{userId, authHash}`. Stores under `pipass.auth.userId` + `pipass.auth.authHash` in `expo-secure-store` (Keychain/Keystore on native, `localStorage` on web — same `Platform.OS === "web"` pattern as `workers/storageWorker.ts`). Exports `getCredentials()`, `setCredentials(creds)`, `clearCredentials()`, `hasCredentials()`, `isValidUserId(v)`, `isValidAuthHash(v)`. Validates UUID + hex format on read AND write; if values on disk fail validation, `getCredentials()` clears them and returns null. **Never cached in memory or React state — `expo-secure-store` is the only trusted credential source.** Never uses `AsyncStorage`. Never logs credential values.
- **`lib/query-client.ts` `authedApiRequest(method, route, data?)`** — request helper for protected endpoints (`/api/vault/sync`, `/api/vault/fetch`). Reads credentials from SecureStore on EVERY call (no caching). Throws `AuthRequiredError` BEFORE any network request if credentials are absent or malformed. Attaches plain-string `x-user-id` and `x-auth-hash` headers (the validators in `getCredentials` make non-string values structurally impossible). On HTTP 401: clears stored credentials and throws `AuthRequiredError` so the caller can route the user to the login flow without first cleaning up. Never logs request headers; the `throwIfResNotOk` helper only includes the server's response body (which never contains `authHash` per the auth security review).
- **`lib/query-client.ts` `apiRequest(method, route, data?)`** — unchanged. Use for unauthenticated endpoints (`/api/auth/register`, `/api/auth/login`, `/api/auth/salt/:username`). After a successful login/register response, persist the returned `id` plus the client-form `authHash` via `setCredentials({ userId: response.id, authHash })`.
- **End-to-end flow verified** (curl + live backend, 2026-04): register → 201 with userId, login → 200, sync → 200, fetch → 200; negative cases (wrong authHash, malformed userId, missing headers, duplicate headers) all → 401 with the expected collapsed error message. Backend log line never contains headers or body — only `METHOD /path STATUS in Xms`.
...
- **Schema** (`shared/schema.ts`): `users` table (uuid PK `defaultRandom`, unique `username`, `authHash`, `salt`, `iterations`, `createdAt` bigint mode:"number") and `vaultBlobs` table (uuid `userId` PK + FK → `users.id` ON DELETE CASCADE, `encryptedBlob`, `version`, `updatedAt` bigint). DB-level CHECK constraints provide defense-in-depth mirrors for persisted rows: `users_iterations_range` enforces `iterations BETWEEN 3 AND 1000000`, and `vault_blobs_version_range` enforces `version >= 1`. The `/api/vault/sync` request schema is strict and accepts `encryptedBlob` plus `expectedPrevVersion` only; stored vault versions are minted by `DatabaseStorage.syncVault()` under the transaction lock. `User`/`NewUser`/`VaultBlob`/`NewVaultBlob` types come from `$inferSelect`/`$inferInsert`.
